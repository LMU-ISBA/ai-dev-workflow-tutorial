...
- [ ] T004 [US1] Add `get_kpis(df)` to `data.py`: returns `{"total_sales": float, "total_orders": int}` computed from the loaded DataFrame
...
- [ ] T006 [US2] Add `get_monthly_trend(df)` to `data.py`: aggregates `total_amount` by calendar month (YYYY-MM), returns a DataFrame with columns `["month", "sales"]` sorted ascending, all 12 months present
